...
public class VerifyBOAMomo extends UssdActionClassInterface {
  static final Logger log = Logger.getLogger("ussd");
  ...
  public TreeMap<String, String> processIntermediateAction(String jsonData) {
    ...
    try {
      ...
      String mmnoCode = m.get("MM_NETWORK");
      String destAcct = m.get("WALLET_NUMBER1");
      log.info("json received:: " + jsonData);
      System.out.println(String.valueOf(destAcct) + "\n" + mmnoCode);
      String response = verifyAccountNumber(destAcct, mmnoCode.split("-")[0]);
  ...
  private String verifyAccountNumber(String accountNumber, String bankCode) {
    ...
    try {
      ...
      log.info("request sent to vasgate:: " + request);
