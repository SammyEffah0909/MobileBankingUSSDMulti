		...
		try {
			...
			if (transType.equals("TAMMC")) {
				...
				if (vastype.equalsIgnoreCase("self")) {
					walletNum = mn;
					elevy = fetchFees(Double.parseDouble(amount), senderBankCode, accountToDebit, mn, bankCode,
				...
				elevy = fetchFees(Double.parseDouble(amount), srcbankCode, mn, mn, targetbankCode, accountToCredit,
						transType).split("[|]");

				if (elevy[0].equals("00") || elevy[0].equals("01")) {
					msg = String.format("Transfer GHS%s to %s", amount, accountToCredit);
					msg += " Elevy: " + elevy[1];
					ref = elevy[2];
					if (elevy[0].equals("01")) {
						msg += "~Overcharged elevy fees will be reversed";
